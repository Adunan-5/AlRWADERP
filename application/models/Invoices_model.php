...
class Invoices_model extends App_Model
{
    ...
    public function send_invoice_to_client($id, $template_name = '', $attachpdf = true, $cc = '', $manually = false, $attachStatement = [], $send_to_email = '')
    {
        $originalNumber = null;
        if ($isDraft = $this->is_draft($id)) {
            // Update invoice number from draft before sending
        ...
        $use_single_email = !empty($send_to_email);

        // Common: Update status early (but we'll revert on full failure)
        $status_updated = update_invoice_status($invoice->id, true, true);
        $invoice_number = format_invoice_number($invoice->id);

        // Common: Generate invoice PDF if attached
        $attach = null;
        if ($attachpdf) {
            set_mailing_constant();
            $pdf = invoice_pdf($this->get($id));
        ...
        // Common: Generate statement PDF if attached
        $attachStatementPdf = false;
        ...
        try {
            if (!$use_single_email && is_array($send_to) && count($send_to) > 0) {
                // Existing contacts logic (loop over multiple)
                $i = 0;
                foreach ($send_to as $contact_id) {
                    if ($contact_id != '') {
                        // Send cc only for the first contact
                        ...
                        $additional_attachments = $this->input->post('email_attachments');
                        if ($additional_attachments) {
                            foreach ($additional_attachments as $attachmentid) {
                                $this_attachment = $this->get_invoice_attachments($id, $attachmentid);
                                $path = path_sales_attachments() . $this_attachment->rel_id . '/' . $this_attachment->file_name;
                                $template->add_attachment([
                                    'attachment' => file_get_contents($path),
                                    'filename' => $this_attachment->file_name,
                                    'type' => get_mime_by_extension($this_attachment->file_name),
                                ]);
                            }
                        }
            ...
            } elseif ($use_single_email) {
                // New single email logic: Split comma-separated emails and loop over each
                $emails = array_map('trim', explode(',', $send_to_email)); // Split and trim whitespace
                $i = 0;
                foreach ($emails as $email) {
                    if (!empty($email)) {
                        // Send cc only for the first email (mimic contacts behavior)
                        $current_cc = (!empty($cc) && $i > 0) ? '' : $cc;

                        // Create fake contact for this single email
                        $fake_contact = (object) [
                            'id' => 0,  // Fake ID, not used for sending
                            'email' => $email,  // Single email only
                            'firstname' => '',  // Optional: Can set to client name if needed
                            'lastname' => '',
                            // Add other fields if template personalizes heavily (e.g., from real client)
                        ];

                        $template = mail_template($template_name, $invoice, $fake_contact, $current_cc);
                        ...
                        // Additional attachments (same as above)
                        $additional_attachments = $this->input->post('email_attachments');
                        if ($additional_attachments) {
                            foreach ($additional_attachments as $attachmentid) {
                                $this_attachment = $this->get_invoice_attachments($id, $attachmentid);
                        ...
                        if ($template->send()) {
                            array_push($emails_sent, $email);
                        } else {
                            // If any single send fails, treat as full failure (clear array)
                            $emails_sent = [];
                            break;  // Stop sending further emails
                        }
                        $i++;
                    }
                }
            ...
            // Log the error for debugging (optional: use log_message('error', $e->getMessage()); )
            // Ensure no emails are marked as sent on exception
            $emails_sent = [];
        } finally {
            // Always clean up if draft and failure
            if ($isDraft && count($emails_sent) == 0 && $status_updated !== false) {
                $this->decrement_next_number();
                $this->db->where('id', $invoice->id);
                $this->db->update('invoices', [
                    'status' => self::STATUS_DRAFT,
