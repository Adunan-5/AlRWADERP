...
class Estimates_model extends App_Model
{
    ...
    public function send_estimate_to_client($id, $template_name = '', $attachpdf = true, $cc = '', $manually = false, $send_to_email = '')
    {
        $estimate = $this->get($id);
        ...
        // Auto-update draft -> sent
        if ($status_now == 1) {
            $this->db->where('id', $estimate->id);
            $this->db->update(db_prefix() . 'estimates', ['status' => 2]);
            $status_auto_updated = true;
        }
        ...
        try {
            if (!$use_single_email && is_array($send_to) && count($send_to) > 0) {
                ...
                $emails = array_map('trim', explode(',', $send_to_email));
                $i = 0;
                foreach ($emails as $email) {
                    if (!empty($email)) {
                        ...
                        $template = mail_template($template_name, $estimate, $fake_contact, $current_cc);
            ...
            log_message('error', 'Estimate send failed: ' . $e->getMessage());
            $emails_sent = [];
        }
        ...
        if ($status_auto_updated) {
            // revert back to draft if send failed
            $this->db->where('id', $estimate->id);
            $this->db->update(db_prefix() . 'estimates', ['status' => 1]);
        }
