...
class Misc_model extends App_Model
{
    ...
    public function _search_staff($q, $limit = 0)
    {
        ...
        if (staff_can('view', 'staff')) {
            ...
            // Searchable fields
            $this->db->group_start();
            $this->db->like(db_prefix() . 'staff.name', $q);
            $this->db->or_like(db_prefix() . 'staff.name_arabic', $q);
            $this->db->or_like(db_prefix() . 'staff.code', $q);
            $this->db->or_like(db_prefix() . 'staff.email', $q);
            $this->db->or_like(db_prefix() . 'staff.phonenumber', $q);
            $this->db->or_like(db_prefix() . 'staff.iqama_number', $q);
            $this->db->or_like(db_prefix() . 'staff.passport_number', $q);
            $this->db->or_like(db_prefix() . 'staff.aramcoid', $q);
            $this->db->or_like(db_prefix() . 'staff.labor_number', $q);
            $this->db->or_like(db_prefix() . 'staff.border_number', $q);
            $this->db->group_end();
            ...
            $this->db->order_by(db_prefix() . 'staff.name', 'ASC');
            $result['result'] = $this->db->get()->result_array();
        }
    ...
    public function _search_projects($q, $limit = 0, $where = false)
    {
        ...
        $this->db->select(db_prefix() . 'projects.id, ' . db_prefix() . 'projects.name, ' . db_prefix() . 'projects.description, ' . db_prefix() . 'projects.clientid, '  . db_prefix() . 'clients.company');
        ...
        if (!startsWith($q, '#')) {
            $this->db->where('('
                ...
                . ' OR ' . db_prefix() . 'projects.description LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\''
                . ' OR ' . db_prefix() . 'projects.name LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\''
            ...
            $this->db->where(db_prefix() . 'projects.id IN
                (SELECT rel_id FROM ' . db_prefix() . 'taggables WHERE tag_id IN
                (SELECT id FROM ' . db_prefix() . 'tags WHERE name="' . $this->db->escape_str(strafter($q, '#')) . '")
