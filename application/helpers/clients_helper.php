...
if (!function_exists('get_client_outstanding_info')) {
    /**
     * Get outstanding amount info for a client
     *
     * @param int $clientid
     * @return array
     * [
     ...
     * ]
     */
    function get_client_outstanding_info($clientid)
    {
        $CI = &get_instance();
        $CI->load->database();

        // Get total invoiced amount
        $CI->db->select('SUM(total) as total_invoiced');
        $CI->db->where('clientid', $clientid);
        $CI->db->from(db_prefix() . 'invoices');
        $total_invoiced = $CI->db->get()->row()->total_invoiced;
        $total_invoiced = $total_invoiced ?? 0;

        // Get total paid amount
        $CI->db->select('SUM(amount) as total_paid');
        $CI->db->where('invoiceid IN (SELECT id FROM ' . db_prefix() . 'invoices WHERE clientid=' . intval($clientid) . ')', null, false);
        $CI->db->from(db_prefix() . 'invoicepaymentrecords');
        $total_paid = $CI->db->get()->row()->total_paid;
        $total_paid = $total_paid ?? 0;

        // Calculate percentage paid
        $percent = $total_invoiced > 0 ? ($total_paid / $total_invoiced) * 100 : 0;
