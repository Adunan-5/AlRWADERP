...
class Projects extends AdminController
{
    ...
    public function project($id = '')
    {
        ...
        if (!$id && $this->input->get('staff_id')) {
            $data['selected_staff'] = $this->input->get('staff_id');
        }
    ...
    // Added by Adunan 
    public function members_table($project_id)
    {
        ...
        $this->app->get_table_data('project_members', [
    ...
    public function assign_job()
    {
        ...
        if ($this->input->post()) {
            $data = $this->input->post();
            $id   = isset($data['edit_id']) ? $data['edit_id'] : null;

            // Check for duplicate assignment (only if adding new)
            if (!$id) {
                $exists = $this->db->where('project_id', $data['project_id'])
            ...
            $save = [
                ...
                'end_date'       => to_sql_date($data['end_date']),
            ...
            // if ($id) {
            //     $this->projectassignee_model->update($id, $save);
            //     set_alert('success', 'Job updated successfully');
            // } else {
            //     $this->projectassignee_model->add($save);
            //     set_alert('success', 'Job assigned successfully');
            // }
            ...
            // redirect(admin_url('staff/member/' . $data['staff_id'] . '#staff_projects'));
    ...
    //         if ($id) {
    //             $this->projectassignee_model->update($id, $save);
    //             set_alert('success', 'Project member updated successfully');
    //         } else {
    //             $this->projectassignee_model->add($save);
    //             set_alert('success', 'Project member added successfully');
    //         }

    //         redirect(admin_url('projects/view/' . $data['project_id'] . '?group=project_members'));
    //     }
    // }
    ...
    public function assign_member() 
    {
        ...
        if ($this->input->post()) {
            ...
            if ($exists && !$id) { // $id null means adding new
                echo json_encode([
                    'success' => false,
                    'message' => 'This employee is already assigned to this project'
                ]);
    ...
    public function delete_member() 
    {
        ...
        if ($id) {
            $this->projectassignee_model->delete($id);
    ...
    public function get_staff_details($staff_id)
    {
        ...
        if ($staff) {
            ...
            // Return data prioritizing PAY tab values, fallback to old fields if no pay data
            echo json_encode([
                'success' => true,
                'basics' => $current_pay ? $current_pay->basic_pay : ($staff->basics ?? ''),
                'ot' => $current_pay ? $current_pay->overtime_pay : ($staff->ot ?? ''),
                'professiontype_id' => $staff->professiontype_id ?? '', // e.g. "2,3,5"
    ...
    /**
     * Download project members import template (Excel/CSV)
     */
    public function download_members_template($project_id)
    {
        if (!has_permission('projects', '', 'view')) {
            access_denied('projects');
        }

        require_once(APPPATH . 'libraries/import/Import_project_members.php');

        $import = new Import_project_members();

        // Set database fields for import
        $import->setDatabaseFields([
            'employee_name',
            'iqama_number',
            'skills',
            'badge',
        ...
        $import->setProjectId($project_id);
        $import->downloadSample();
    }

    /**
     * Upload file and show preview of project members import
     */
    public function bulk_upload_members_preview($project_id)
    {
        ...
        if ($this->input->post() && isset($_FILES['file_csv']['name']) && $_FILES['file_csv']['name'] != '') {
            ...
            $file_extension = strtolower(pathinfo($_FILES['file_csv']['name'], PATHINFO_EXTENSION));
            ...
            if ($_FILES['file_csv']['error'] !== UPLOAD_ERR_OK) {
                ...
                $error_message = isset($upload_errors[$_FILES['file_csv']['error']])
                    ? $upload_errors[$_FILES['file_csv']['error']]
            ...
            // Ensure base TEMP_FOLDER exists
            if (!is_dir(TEMP_FOLDER)) {
                if (!mkdir(TEMP_FOLDER, 0755, true)) {
                    echo json_encode(['success' => false, 'message' => 'Failed to create base temporary directory: ' . TEMP_FOLDER . '. Check folder permissions.']);
                    return;
                }
            }

            // Save uploaded file to temp directory for later reuse
            $temp_dir = TEMP_FOLDER . 'bulk_import_' . time() . '_' . uniqid() . '/';
            if (!is_dir($temp_dir)) {
                if (!mkdir($temp_dir, 0755, true)) {
                    echo json_encode(['success' => false, 'message' => 'Failed to create temporary directory. Check folder permissions for: ' . TEMP_FOLDER]);
                    return;
                }
            }

            $temp_file_path = $temp_dir . $_FILES['file_csv']['name'];

            if (!move_uploaded_file($_FILES['file_csv']['tmp_name'], $temp_file_path)) {
            ...
            $import->setProjectId($project_id);
            $import->setSimulation(true);
            $import->setFilename($_FILES['file_csv']['name']);
            $import->setTemporaryFileLocation($temp_file_path);
            ...
            // Store import data in session for processing later
            $this->session->set_userdata('bulk_import_data', [
                'project_id' => $project_id,
                ...
                'simulation_data' => $import->getSimulationData()
            ]);
            ...
            echo json_encode([
                ...
                'redirect_url' => admin_url('projects/bulk_upload_members_preview_page/' . $project_id)
            ]);
        } else {
            echo json_encode([
                'success' => false,
    ...
    /**
     * Show the preview page after upload
     */
    public function bulk_upload_members_preview_page($project_id)
    {
        if (!has_permission('projects', '', 'create') && !has_permission('projects', '', 'edit')) {
            access_denied('projects');
        }
        ...
        $import_data = $this->session->userdata('bulk_import_data');

        if (!$import_data || $import_data['project_id'] != $project_id) {
            set_alert('danger', _l('bulk_import_session_expired'));
            redirect(admin_url('projects/view/' . $project_id . '?group=project_members'));
        }

        $data['project_id'] = $project_id;
        $data['simulation_data'] = $import_data['simulation_data'];
        $data['validation_errors'] = $import_data['validation_errors'];
        $data['total_rows'] = $import_data['total_rows'];
        $data['total_valid'] = $import_data['total_rows'] - count($import_data['validation_errors']);
        $data['total_errors'] = count($import_data['validation_errors']);
        $data['title'] = _l('bulk_upload_project_members_preview');

        $this->load->view('admin/projects/bulk_upload_preview', $data);
    }

    /**
     * Process the bulk import after preview confirmation
     */
    public function bulk_upload_members_process($project_id)
    {
        if (!has_permission('projects', '', 'create') && !has_permission('projects', '', 'edit')) {
            access_denied('projects');
        }

        $skip_errors = $this->input->post('skip_errors') ? true : false;
        ...
        if (!isset($import_data['temp_file_path']) || !file_exists($import_data['temp_file_path'])) {
            set_alert('danger', _l('import_upload_failed'));
            redirect(admin_url('projects/view/' . $project_id . '?group=project_members'));
        }
        ...
        $import->setProjectId($project_id);
        $import->setSimulation(false); // Actual import
        $import->setFilename($import_data['filename']);
        $import->setTemporaryFileLocation($import_data['temp_file_path']);

        // Get selected rows from POST data (if checkbox selection is used)
        $selected_rows = $this->input->post('selected_rows');
        if ($selected_rows && is_array($selected_rows)) {
            $import->setSelectedRows($selected_rows);
        }
        ...
        try {
            ...
            // Clean up temp file on error
            if (file_exists($import_data['temp_file_path'])) {
                @unlink($import_data['temp_file_path']);
            }
            if (is_dir($import_data['temp_dir'])) {
                @rmdir($import_data['temp_dir']);
            }

            // Clear session data
            $this->session->unset_userdata('bulk_import_data');

            set_alert('danger', 'Import failed: ' . $e->getMessage());
            redirect(admin_url('projects/view/' . $project_id . '?group=project_members'));
        }

        $totalImported = $import->totalImported();
        $totalErrors = count($import->getValidationErrors());
        ...
        if ($totalImported > 0) {
            set_alert('success', sprintf(_l('bulk_import_success'), $totalImported));
        }

        if ($totalErrors > 0 && !$skip_errors) {
            set_alert('warning', sprintf(_l('bulk_import_errors_found'), $totalErrors));
        }
