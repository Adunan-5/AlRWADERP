...
class Estimates extends AdminController
{
    ...
    public function send_to_email($id)
    {
        ...
        try {
            // Capture single email input from UI
            $send_to_email = $this->input->post('send_to_email');

            // Send estimate using model (with new email param)
            $success = $this->estimates_model->send_estimate_to_client(
                $id,
                '', // blank 'send_to' param (we’ll use the new one inside model)
                $this->input->post('attach_pdf'),
                $this->input->post('cc'),
                false,
                $send_to_email // pass the single email to model
            );
        } catch (Exception $e) {
            $message = $e->getMessage();
            echo $message;
