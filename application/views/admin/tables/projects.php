...
return App_table::find('projects')
    ->outputUsing(function ($params) {
        ...
        $aColumns = [
            ...
            ON ' . db_prefix() . 'staff.staffid = ' . db_prefix() . 'projectassignee.staff_id 
            WHERE ' . db_prefix() . 'projectassignee.project_id=' . db_prefix() . 'projects.id 
            ORDER BY ' . db_prefix() . 'projectassignee.staff_id) as members',
            'status',
        ];
        ...
        if (staff_cant('view', 'projects')) {
            array_push($where, ' AND ' . db_prefix() . 'projects.id IN (
                SELECT ' . db_prefix() . 'projectassignee.project_id
                FROM ' . db_prefix() . 'projectassignee 
                WHERE ' . db_prefix() . 'projectassignee.staff_id=' . get_staff_user_id() . ')'
            );
        }
        ...
        $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
            'clientid',
            '(SELECT GROUP_CONCAT(staff_id SEPARATOR ",") FROM ' . db_prefix() . 'projectassignee WHERE ' . db_prefix() . 'projectassignee.project_id=' . db_prefix() . 'projects.id 
  ORDER BY ' . db_prefix() . 'projectassignee.staff_id) as members_ids',
        ]);
        ...
        foreach ($rResult as $aRow) {
            ...
            $link = admin_url('projects/view/' . $aRow['id'] . '/?group=project_timesheets');

            $row[] = '<a href="' . $link . '" class="tw-font-medium">' . $aRow['id'] . '</a>';

            $name = '<a href="' . $link . '" class="tw-font-medium">' . e($aRow['project_name']) . '</a>';
            ...
            if ($hasPermissionCreate && !$clientid) {
                $name .= ' | <a href="#" data-name="' . e($aRow['project_name']) . '" onclick="copy_project(' . $aRow['id'] . ', this);return false;">' . _l('copy_project') . '</a>';
            }
            ...
            $limit = 5; // Show only 5 members
            $totalMembers = count(array_filter($members));
            $shown = 0;

            foreach ($members as $key => $member) {
                if ($member != '' && $shown < $limit) {
                    $member_id   = $members_ids[$key];
                    $membersOutput .= '<a href="' . admin_url('staff/member/' . $member_id) . '">' .
            ...
            // If there are hidden members
            if ($totalMembers > $limit) {
                $extra = $totalMembers - $limit;
                $membersOutput .= '<span class="tw-inline-flex tw-items-center tw-justify-center tw-h-7 tw-w-7 tw-rounded-full tw-bg-neutral-200 tw-text-xs tw-font-medium">+' . $extra . '</span>';
            }
        ...
        App_table_filter::new('project_name','TextRule')->label(_l('project_name')),
        ...
        App_table_filter::new('members', 'MultiSelectRule')->label(_l('project_members'))
            ...
            ->options(function ($ci) {
                ...
                return "({$dbPrefix}projects.id IN (
                    SELECT {$dbPrefix}projectassignee.project_id 
                    FROM {$dbPrefix}projectassignee
                    WHERE {$dbPrefix}projectassignee.staff_id $sqlOperator ('" . implode("','", $value) . "')
                ))";
            })
    ]);
