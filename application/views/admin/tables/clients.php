...
return App_table::find('clients')
    ->outputUsing(function ($params) {
        ...
        $aColumns = [
            ...
            // Projects column: numbered links with refined green styling
            '(SELECT GROUP_CONCAT(
                    CONCAT(
                        p.id, "||", 
                        p.name, "||", 
                        IFNULL((SELECT COUNT(*) FROM ' . db_prefix() . 'projectassignee pa WHERE pa.project_id = p.id), 0)
                    )
                    SEPARATOR "%%"
                )
                FROM ' . db_prefix() . 'projects p
                WHERE p.clientid = ' . db_prefix() . 'clients.userid
            ) as clientProjects',
        ...
        foreach ($rResult as $aRow) {
            ...
            $company .= '<a href="' . admin_url('clients/client/' . $aRow['userid'] . ($isPerson && $aRow['contact_id'] ? '?group=contacts' : '')) . '">' . _l('edit') . '</a>';
            ...
            // Outstanding info
            $outstanding = get_client_outstanding_info($aRow['userid']);
            $paid_formatted  = number_format($outstanding['paid'], 2) . ' SAR';
            $total_formatted = number_format($outstanding['total'], 2) . ' SAR';
            $percent         = round($outstanding['percent'], 2);

            $progress_class = 'bg-danger'; // default
            ...
            $progress_html = '<div class="progress" style="height:6px; margin-top:2px;">
                                <div class="progress-bar ' . $progress_class . '" role="progressbar" style="width:' . $percent . '%;"></div>
                            </div>';

            $row[] = '<div class="outstanding-info small">
                        <strong>' . $paid_formatted . '</strong> (Amount received)<br>
                        ' . $percent . '% of total invoiced <strong>' . $total_formatted . '</strong><br>
                        ' . $progress_html . '
                    </div>';
            ...
            // $row[] = $aRow['clientProjects'] ?? '';
            // Refine projects display
            $projects_html = '';
            if (!empty($aRow['clientProjects'])) {
                $projects = explode('%%', $aRow['clientProjects']);
                $counter = 1;

                foreach ($projects as $projData) {
                    list($projId, $projName, $empCount) = explode('||', $projData);
                    $empLabel = $empCount . ' Employee' . ($empCount > 1 ? 's' : '');
                    $projectUrl = admin_url('projects/view/' . $projId . '?group=project_timesheets');
                    
                    $projects_html .= $counter . '. <a href="' . $projectUrl . '" style="color: #28a745; text-decoration: none; font-weight: 500;">' 
                                    . e($projName) . ' (' . $empLabel . ')</a><br>';
                    $counter++;
                }

                $projects_html = '<div class="projects-list small text-success" style="line-height: 1.4;">' . $projects_html . '</div>';
            }
            $row[] = $projects_html;
            $row[] = $toggleActive;
