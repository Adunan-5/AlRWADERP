...
$staffs = array_values(array_filter($staffs, function ($staff) {
		...
		foreach ($staffs as $staff_key => $staff_value) {
			...
			if ($rel_type == 'hr_records') {
				...
				// 	$data_object_kpi[$staff_key]['social_security_no'] = $employees_value[$staff_value['staffid'] . '_' . $current_month]['social_security_no'];
					...
					$data_object_kpi[$staff_key]['bank_code'] = $employees_value[$staff_value['staffid'] . '_' . $current_month]['bank_code'];
				...
				if ($rel_type == 'hr_records') {
					...
					if (isset($employees_value[$staff_value['staffid'] . '_' . $month_filter])) {
						...
						$data_object_kpi[$staff_key]['employee_account_no_iban'] = !empty($employees_value[$staff_value['staffid'] . '_' . $month_filter]['employee_account_no_iban'])
																		...
																		: $staff_value['bank_iban_number'];
						$data_object_kpi[$staff_key]['bank_code'] = $employees_value[$staff_value['staffid'] . '_' . $month_filter]['bank_code'];
	...
	public function import_employees_excel()
    {
		...
		// Log headers
        log_message('debug', 'Excel Headers: ' . json_encode($headers));
        ...
        $total_row_success = 0;
        $total_row_false = 0;
        $error_rows = [$headers]; // Include headers in error file
        $arr_insert = [];
        $arr_update = [];
	...
	public function generate_payslip($staff_id, $month, $return = false)
	{
		...
		$payment = $this->db->get_where('tblhrp_payments', [
			'staff_id' => $staff_id,
			'month'    => $db_month,
			'status'   => 'paid'
		])->row_array();

		$is_paid = !empty($payment); // true if a payment exists with 'paid' status
		$paid_date = $is_paid ? $payment['paid_date'] : (clone $month_date)->modify('+1 month')->format('Y-m-d');
		$paid_amount = $is_paid ? floatval($payment['amount']) : 0;
		...
		$month_end = (clone $month_date)->modify('last day of this month');
		$period = $month_date->format('Y-m-d') . ' - ' . $month_end->format('Y-m-d') . ' (' . $month_date->format('F, Y') . ')';

		// Employee info
		$infoRows = [
			['Period:', $period, 'Job Title:', $employee_value['job_title'] ?? 'N/A'],
			['Name:', $employee['name'] ?? 'N/A', 'Iqama #:', $employee['iqama_number'] ?? 'N/A'],
			['Bank:', $employee['bank_name'] ?? 'N/A', 'Bank A/C #:', $employee['bank_account_number'] ?? 'N/A'],
		...
		foreach ($adjustments as $adj) {
			if ($adj['type'] === 'addition') {
				...
				$adj_add_rows[] = [$adj['description'] ?? 'Adjustment Addition', number_format($adj['amount'], 2) . ' SAR'];
			} elseif ($adj['type'] === 'deduction') {
				$adj_ded_total += floatval($adj['amount']);
				$adj_ded_rows[] = [$adj['description'] ?? 'Adjustment Deduction', '-' . number_format($adj['amount'], 2) . ' SAR'];
			}
		}

		// Totals
		$total_additions = $allowance + $adj_add_total;
		$total_deductions = $deduction + $adj_ded_total;
		$net_salary = $gross + $total_additions - $total_deductions;

		// Days in month for daily salary calculation
		$days_in_month = (int)$month_date->format('t');
		$daily_salary = $days_in_month > 0 ? $basic / $days_in_month : 0;

		// Convert net salary to words
		$amount_in_words = $this->number_to_words($net_salary) . ' SAR Only';

		$rows = [
			['Basic', number_format($basic, 2) . ' SAR'],
			['', "Dates 1..$days_in_month ($days_in_month days) Monthly " . number_format($basic, 2) . " SAR Daily " . number_format($daily_salary, 2) . " SAR"],
			['Overtime', number_format($ot_amount, 2) . ' SAR'],
			['', "($ot_hours Hours x " . number_format($ot_rate, 2) . " SAR)"],
			['Gross Salary', number_format($gross, 2) . ' SAR'],

			// Allowance / Deduction base lines
			['Additions', $allowance > 0 ? ($employee_value['mention'] ?? 'Salary Addition: ') . number_format($allowance, 2) . ' SAR' : 'N/A'],
		];

		// Only show adjustment additions if available
		if ($adj_add_total > 0) {
			foreach ($adj_add_rows as $ar) {
				$rows[] = ['   → ' . $ar[0], $ar[1]]; // indent with arrow
			}
			$rows[] = ['Total Additions', number_format($total_additions, 2) . ' SAR'];
		}

		$rows[] = ['Deductions', $deduction > 0 ? ($employee_value['mention'] ?? 'Deduction: ') . '-' . number_format($deduction, 2) . ' SAR' : 'N/A'];

		// Only show adjustment deductions if available
		if ($adj_ded_total > 0) {
			foreach ($adj_ded_rows as $dr) {
				$rows[] = ['   → ' . $dr[0], $dr[1]];
			}
			$rows[] = ['Total Deductions', '-' . number_format($total_deductions, 2) . ' SAR'];
		}
		...
		// Paid On row
		$label = 'Paid On: ' . $paid_date;
		$value = $is_paid ? number_format($paid_amount, 2) . ' SAR' : 'N/A';

		$rows[] = [$label, $value, 'PAID', $is_paid]; // we pass PAID badge flag as 4th element
		...
		foreach ($rows as $row) {
			...
			if (isset($row[3]) && $row[3] === true) { // Paid row with badge
				$rowHeight = 8;
				$x = $pdf->GetX();
				$y = $pdf->GetY();
				...
				// Value (amount + PAID badge inline)
				$pdf->SetFont('dejavusans', '', 10);
				...
				$pdf->Rect($x + $col1Width + $amountWidth + 2, $y, $badgeWidth, $badgeHeight, 'F'); // small gap + filled rectangle
				$pdf->SetFont('dejavusans', 'B', 8); // B = bold, 8 = font size
				$pdf->SetXY($x + $col1Width + $amountWidth + 2, $y);
				$pdf->Cell($badgeWidth, $badgeHeight, 'PAID', 0, 0, 'C');
	...
	private function number_to_words($number)
	{
		...
		if ($number < 21) {
			...
			$string = $this->number_to_words($thousands) . ' ' . $dictionary[1000];
			if ($remainder) {
				$string .= $separator . $this->number_to_words($remainder);
			}
		} else {
			$millions = (int)($number / 1000000);
			$remainder = $number % 1000000;
			$string = $this->number_to_words($millions) . ' ' . $dictionary[1000000];
			if ($remainder) {
				$string .= $separator . $this->number_to_words($remainder);
			}
		}
	...
	public function get_preview($staff_id, $month)
	{
		...
		// --- Base deduction from employee values ---
		$baseDeduction = (float)($val['deduction'] ?? 0);

		// --- Adjustments (list + sums) ---
		$adjustments = $this->db->where(['staff_id' => $staff_id, 'month' => $db_month])
			->get('tblhrp_adjustments')->result_array();

		$sumAdd = 0; $sumDed = $baseDeduction;
		$addRows = ''; $dedRows = '';

		// If there’s a base deduction, display it first
		if ($baseDeduction > 0) {
			$dedRows .= '<tr><td>→ Deduction</td><td>-'.number_format($baseDeduction,2).' SAR</td></tr>';
		}

		foreach ($adjustments as $adj) {
			if ($adj['type'] === 'addition') {
				$sumAdd += (float)$adj['amount'];
				$addRows .= '<tr><td>→ '.htmlspecialchars($adj['description'] ?: 'Adjustment Addition').'</td><td>'.number_format($adj['amount'],2).' SAR</td></tr>';
			}
			if ($adj['type'] === 'deduction') {
				$sumDed += (float)$adj['amount'];
				$dedRows .= '<tr><td>→ '.htmlspecialchars($adj['description'] ?: 'Adjustment Deduction').'</td><td>-'.number_format($adj['amount'],2).' SAR</td></tr>';
			}
		}

		// --- Payments ---
		$paidRow = $this->db->select('SUM(amount) as total_paid, MAX(status) as last_status')
			->where(['staff_id' => $staff_id, 'month' => $db_month])
			->get('tblhrp_payments')->row();
		$total_paid = (float)($paidRow->total_paid ?? 0);
		$is_paid = ($paidRow->last_status === 'paid' && $total_paid >= 0.01);
		...
		<div class="payslip-preview">
			...
			<table class="table table-sm">
				...
				<tr><td colspan="2">Dates 1..<?= $days_in_month ?> (<?= $days_in_month ?> days) — Monthly <?= number_format($basic,2) ?> SAR — Daily <?= number_format($daily,2) ?> SAR</td></tr>

				<tr><td><strong>Overtime</strong></td><td><?=number_format($ot_amount,2)?> SAR</td></tr>
				<tr><td colspan="2">(<?=number_format((float)$val['ot_hours'],2)?> Hours x <?=number_format((float)$val['ot_rate'],2)?> SAR)</td></tr>

				<tr><td><strong>Gross Salary</strong></td><td><?=number_format($gross,2)?> SAR</td></tr>

				<?php if ($addRows): ?>
					<tr class="table-success"><td colspan="2"><strong>Adjustments (Additions)</strong></td></tr>
					<?= $addRows ?>
				...
				<tr>
					<td><strong>To be paid</strong></td>
					<td>
						<strong><?=number_format(max(0, $net - $total_paid),2)?> SAR</strong>
						<?php if ($is_paid): ?>
