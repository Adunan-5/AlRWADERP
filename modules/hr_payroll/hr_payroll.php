...
Author: GrandMaster Consultation
Author URI: https://consultation.ggm.com.co
*/
...
define('HR_PAYROLL_REVISION', 1100);
...
function hr_payroll_module_init_menu_items()
{
    ...
    if(has_permission('hrp_employee','','view') || has_permission('hrp_employee','','view_own')){
        // Mohtarifeen Employees (company_id = 2)
        $CI->app_menu->add_sidebar_children_item('hr_payroll', [
            'slug'     => 'hr_mohtarifeen_employees',
            'name'     => 'Mohtarifeen Employees',
            'icon'     => 'fa fa-vcard',
            'href'     => admin_url('hr_payroll/manage_employees?company=mohtarifeen'),
            'position' => 1,
        ]);

        // Mahiroon Employees (company_id != 2)
        $CI->app_menu->add_sidebar_children_item('hr_payroll', [
            'slug'     => 'hr_mahiroon_employees',
            'name'     => 'Mahiroon Employees',
            'icon'     => 'fa fa-vcard',
            'href'     => admin_url('hr_payroll/manage_employees?company=mahiroon'),
            'position' => 2,
        ]);

        // Payroll List - New menu item for payroll generation system
        $CI->app_menu->add_sidebar_children_item('hr_payroll', [
            'slug'     => 'hr_payroll_list',
            'name'     => _l('hr_payroll_list'),
            'icon'     => 'fa fa-list-alt',
            'href'     => admin_url('hr_payroll/payroll_list'),
            'position' => 3,
        ]);
    }
    ...
    if(has_permission('hrp_setting','','view')){
        ...
        // WPS Export Settings
        $CI->app_menu->add_sidebar_children_item('hr_payroll', [
            'slug'     => 'hrp_wps_settings',
            'name'     => _l('hrp_wps_export_settings'),
            'icon'     => 'fa fa-file-excel-o',
            'href'     => admin_url('hr_payroll/wps_settings'),
            'position' => 12,
        ]);
...
function hr_payroll_permissions()
{
    ...
    register_staff_capabilities('hrp_employee', $capabilities, _l('hr_payroll_employee'));
