...
";
	...
	public function get_employees_data($month, $rel_type ='', $where='')
	{
		...
		// UPDATED: Include records with matching rel_type OR NULL/empty (for timesheet partials)
		$this->db->where("(rel_type = '" . $this->db->escape_str($rel_type) . "' OR rel_type IS NULL OR rel_type = '')");
		
		$this->db->where("date_format(month, '%Y-%m-%d') = '" . $this->db->escape_str($month) . "'");
		$this->db->order_by('staff_id', 'asc');
		$employees = $this->db->get(db_prefix() . 'hrp_employees_value')->result_array();
	...
	/**
	 * Get employee data for a specific staff and month
	 * @param  int $staff_id
	 * @param  string $month
	 * @return array|null
	 */
	public function get_employees_data_by_staff($staff_id, $month)
	{
		$this->db->where('staff_id', $staff_id);
		$this->db->where("date_format(month, '%Y-%m-%d') = '" . $this->db->escape_str($month) . "'");
		$employee = $this->db->get(db_prefix() . 'hrp_employees_value')->row_array();

		if ($employee) {
			$employees_decode = $this->employees_data_json_encode_decode('decode', [$employee], '');
			return $employees_decode[0] ?? null;
		}
	...
	public function employees_data_json_encode_decode($type, $data, $header ='')
	{
		if($type == 'decode'){
			// json decode
			foreach ($data as $key => $value) {
			    // Safe decode with fallback to []
				$probationary_contracts = !empty($value['probationary_contracts'])
					? json_decode($value['probationary_contracts'], true)
					: [];

				$primary_contracts = !empty($value['primary_contracts'])
					? json_decode($value['primary_contracts'], true)
					: [];
	...
	public function get_format_employees_data($rel_type)
	{
		...
		$staff_information[] = 'select'; // Checkbox column for export
		$staff_information[] = 'employee_number';
		$staff_information[] = 'employee_name';
		$staff_information[] = 'employee_id_iqama';
		$staff_information[] = 'employee_account_no_iban';
		$staff_information[] = 'bank_code';
		$staff_information[] = 'gosi_basic_salary';
		$staff_information[] = 'gosi_housing_allowance';
		$staff_information[] = 'gosi_other_allowance';
		$staff_information[] = 'gosi_deduction';
		$staff_information[] = 'total_amount';
		$staff_information[] = 'balance';
		$staff_information[] = 'full_salary';
		$staff_information[] = 'basic';
		$staff_information[] = 'ot_hours';
		$staff_information[] = 'ot_rate';
		$staff_information[] = 'ot_amount';
		$staff_information[] = 'allowance';
		$staff_information[] = 'additions';
		$staff_information[] = 'deduction';
		$staff_information[] = 'payslip';
		$staff_information[] = 'mention';
		$staff_information[] = 'payroll_month';
		$staff_information[] = 'comment_1';
		$staff_information[] = 'comment_2';
		$staff_information[] = 'comment_3';
		...
		foreach ($staff_information as $value) {
			if($value == 'staff_id'){
				...
				$staff_information_header[] = '☑'; // Checkbox symbol for header
			}else{
			...
			// Make payslip column use custom renderer
			if ($value == 'payslip') {
				$col['renderer'] = 'payslipRenderer';
				$col['readOnly'] = true;
			}

			// Define editable fields - only these can be edited in the grid
			$editable_fields = [
				'gosi_other_allowance',
				'gosi_deduction',
				'mention',
				'payroll_month',
				'comment_1',
			...
			// Make all fields read-only except the editable ones
			if (!in_array($value, $editable_fields) && $value != 'select') {
				$col['readOnly'] = true;
			}

			// Apply color coding to different column groups
			// Pale green for GOSI columns
			$gosi_columns = ['gosi_basic_salary', 'gosi_housing_allowance', 'gosi_other_allowance', 'gosi_deduction', 'total_amount'];
			if (in_array($value, $gosi_columns)) {
				$col['className'] = isset($col['className']) ? $col['className'] . ' bg-pale-green' : 'bg-pale-green';
			}

			// Pale orange for balance column
			if ($value == 'balance') {
				$col['className'] = isset($col['className']) ? $col['className'] . ' bg-pale-orange' : 'bg-pale-orange';
			}

			// Pale blue for payslip calculation columns
			$payslip_columns = ['full_salary', 'basic', 'ot_hours', 'ot_rate', 'ot_amount', 'allowance', 'additions', 'deduction'];
			if (in_array($value, $payslip_columns)) {
				$col['className'] = isset($col['className']) ? $col['className'] . ' bg-pale-blue' : 'bg-pale-blue';
			}

			// Pale amber for reference/comment columns
			$reference_columns = ['mention', 'payroll_month', 'comment_1', 'comment_2', 'comment_3'];
			if (in_array($value, $reference_columns)) {
				$col['className'] = isset($col['className']) ? $col['className'] . ' bg-pale-amber' : 'bg-pale-amber';
			}

			array_push($column_format, $col);
		}
...
/*TO DO: XỬ LÝ CẬP NHẬT, cập nhật thông tin nhân viên sau khi người dùng thực hiện thay đổi trên handsome table, dùng cho 2 trường hợp: có tích hợp HR records module, không tích hợp Hr records module.  đang xử lý!!!*/
	...
	public function employees_update($data)
	{
		try {
			...
			if (!isset($data['employees_fill_month'])) {
				throw new Exception('Missing employees_fill_month');
			}
		...
		if(isset($hrp_employees_value)){
			...
			foreach ($hrp_employees_detail as $key => $value) {				
				...
				// Remove the last column "payslip" if present
				...
				foreach ($combine_data as $combine_key => $combine_value) {
					...
					if($rel_type == 'hr_records'){
						...
						}elseif($combine_key != 'department_name' && $combine_key != 'employee_name' && $combine_key != 'employee_number' && $combine_key != 'select' && $combine_key != 'payslip'){
				...
				// CRITICAL: Add payroll_id if in payroll mode to prevent saving without payroll_id
				if (isset($data['payroll_id']) && $data['payroll_id']) {
					$temp['payroll_id'] = $data['payroll_id'];
				}
		...
		$department_filter = $data['department_employees_filter'] ?? '';
		$staff_filter = $data['staff_employees_filter'] ?? '';
		$role_filter = $data['role_employees_filter'] ?? '';
		$payroll_id = $data['payroll_id'] ?? null;

		// Only delete non-visible employees if:
		// 1. NO filters are applied AND
		// 2. NOT in payroll mode (payroll mode should only update specific payroll employees)
		if ($department_filter === '' && $staff_filter === '' && $role_filter === '' && !$payroll_id) {
			$row['delete'] = implode(",", $row['delete']);
			$this->db->where('id NOT IN (' . $row['delete'] . ') AND rel_type = "' . $rel_type . '" AND DATE_FORMAT(month,"%Y-%m-%d") = "' . $employees_month . '"');
			$this->db->delete(db_prefix() . 'hrp_employees_value');
			if ($this->db->affected_rows() > 0) {
		...
		if(count($row['update']) != 0){
			...
			log_message('error', 'employees_update error: ' . $e->getMessage() . ' | Line: ' . $e->getLine());
	...
	public function import_employees_data($es_detail)
    {
        ...
        foreach ($es_detail as $employee) {
            // Ensure all required fields are present
            $data = [
                'staff_id' => $employee['staff_id'],
                'month' => $employee['month'],
                'employee_id_iqama' => $employee['employee_id_iqama'],
                'employee_account_no_iban' => $employee['employee_account_no_iban'],
                'employee_name' => $employee['employee_name'],
                'bank_code' => $employee['bank_code'],
                'gosi_basic_salary' => $employee['gosi_basic_salary'],
                'gosi_housing_allowance' => $employee['gosi_housing_allowance'],
                'gosi_other_allowance' => $employee['gosi_other_allowance'],
                'gosi_deduction' => $employee['gosi_deduction'],
                'total_amount' => $employee['total_amount'],
                'balance' => $employee['balance'],
                'full_salary' => $employee['full_salary'],
        ...
        if (count($row['insert']) > 0) {
            $affectedRows += $this->db->insert_batch(db_prefix() . 'hrp_employees_value', $row['insert']);
        }
        if (count($row['update']) > 0) {
            $affectedRows += $this->db->update_batch(db_prefix() . 'hrp_employees_value', $row['update'], 'id');
        }
	...
	public function get_staff_payslip_data($staff_id)
	{
		return $this->db->where('staffid', $staff_id)->get(db_prefix().'staff')->row();
	}

	public function get_total_ot_hours($staff_id, $month_date) 
	{
		// $month_date is like '2025-09-01' (first of month)
		$this->db->select_sum('total_overtime_hours');
		$this->db->from('tbltimesheet_master');
		$this->db->where('staff_id', $staff_id);
		$this->db->where('month_year', $month_date);  // Exact match to first-of-month
		$query = $this->db->get();
		$result = $query->row();
		return $result->total_overtime_hours ?? 0.00;  // Return 0.00 if no records
	}

	public function get_net_adjustment($staff_id, $month)
	{
		$net = $this->db->select('SUM(CASE WHEN type = "addition" THEN amount ELSE -amount END) as net_adjustment')
						->where('staff_id', $staff_id)
						->where('DATE(month)', $month) // Matches first-of-month
						->get(db_prefix() . 'hrp_adjustments')
						->row('net_adjustment');
		return floatval($net ?? 0);
	}

	/**
	 * Get total additions (allowances) from hrp_adjustments table for a specific month
	 * @param int $staff_id Staff ID
	 * @param string $month Month in Y-m-d format (e.g., 2025-01-01)
	 * @return float Total additions
	 */
	public function get_additions_for_month($staff_id, $month)
	{
		$result = $this->db->select('SUM(amount) as total')
			->from(db_prefix() . 'hrp_adjustments')
			->where('staff_id', $staff_id)
	...
	/**
	 * Get total deductions from hrp_adjustments table for a specific month
	 * @param int $staff_id Staff ID
	 * @param string $month Month in Y-m-d format (e.g., 2025-01-01)
	 * @return float Total deductions
	 */
	public function get_deductions_for_month($staff_id, $month)
	{
		$result = $this->db->select('SUM(amount) as total')
			->from(db_prefix() . 'hrp_adjustments')
			->where('staff_id', $staff_id)
	...
	/**
	 * Get staff pay data from tblstaffpay for a specific month
	 * Returns the most recent pay record where start_date <= month
	 * @param int $staff_id Staff ID
	 * @param string $month Month in Y-m-d format (e.g., 2025-01-01)
	 * @return object|null Pay record or null if not found
	 */
	public function get_staff_pay_for_month($staff_id, $month)
	{
		...
		$this->db->select('*');
		$this->db->from(db_prefix() . 'staffpay');
		$this->db->where('staff_id', $staff_id);
		// Get records that started before or during this month
	...
	/**
	 * Get project-based additions (allowances) for staff in a specific month
	 * @param int $staff_id Staff ID
	 * @param string $month Month in Y-m-d format (e.g., 2025-01-01)
	 * @return float Total allowances from all projects
	 */
	public function get_project_additions_for_month($staff_id, $month)
	{
		$result = $this->db->select('SUM(amount) as total')
	...
	/**
	 * Get project-based deductions for staff in a specific month
	 * @param int $staff_id Staff ID
	 * @param string $month Month in Y-m-d format (e.g., 2025-01-01)
	 * @return float Total deductions from all projects
	 */
	public function get_project_deductions_for_month($staff_id, $month)
	{
		$result = $this->db->select('SUM(amount) as total')
			->from(db_prefix() . 'timesheet_adjustments')
			->where('staff_id', $staff_id)
	...
	/**
	 * Get all custom allowances applicable to an employee based on their staff type, company type, and profession type
	 * Uses OR logic: Returns allowances if employee matches ANY of the assigned types
	 * @param int $staff_id Staff ID
	 * @return array Array of allowance types with assignment details
	 */
	public function get_employee_applicable_allowances($staff_id)
	{
		$CI = &get_instance();
		...
		$allowances = [];
		$allowance_ids = []; // Track IDs to prevent duplicates

		// 1. Check Staff Type assignments
		if (!empty($staff->stafftype_id)) {
			$this->db->select('at.*, aa.is_mandatory, aa.default_amount');
			$this->db->from(db_prefix() . 'allowance_types at');
			$this->db->join(db_prefix() . 'allowance_assignments aa', 'aa.allowance_type_id = at.id');
			$this->db->where('aa.employee_type', 'staff_type');
			$this->db->where('aa.employee_type_id', $staff->stafftype_id);
			$this->db->where('at.is_active', 1);
			$staff_type_allowances = $this->db->get()->result_array();
		...
		// 2. Check Company Type assignments
		if (!empty($staff->companytype_id)) {
			$this->db->select('at.*, aa.is_mandatory, aa.default_amount');
			$this->db->from(db_prefix() . 'allowance_types at');
			$this->db->join(db_prefix() . 'allowance_assignments aa', 'aa.allowance_type_id = at.id');
			$this->db->where('aa.employee_type', 'company_type');
			$this->db->where('aa.employee_type_id', $staff->companytype_id);
			$this->db->where('at.is_active', 1);
			$company_allowances = $this->db->get()->result_array();
		...
		// 3. Check Profession Type assignments
		if (!empty($staff->professiontype_id)) {
			$this->db->select('at.*, aa.is_mandatory, aa.default_amount');
			$this->db->from(db_prefix() . 'allowance_types at');
			$this->db->join(db_prefix() . 'allowance_assignments aa', 'aa.allowance_type_id = at.id');
			$this->db->where('aa.employee_type', 'profession_type');
			$this->db->where('aa.employee_type_id', $staff->professiontype_id);
			$this->db->where('at.is_active', 1);
			$profession_allowances = $this->db->get()->result_array();
	...
	/**
	 * Calculate GOSI Other Allowance (excludes GOSI Basic and GOSI Housing)
	 * Includes: food, general allowance, fat, MEWA, and custom allowances
	 * @param int $staff_id Staff ID
	 * @param string $month Month in Y-m-d format (e.g., 2025-01-01)
	 * @return float Total GOSI other allowances
	 */
	public function calculate_gosi_other_allowance($staff_id, $month, $proration_factor = 1.0)
	{
		// Get fixed allowances from tblstaffpay
		$staff_pay = $this->get_staff_pay_for_month($staff_id, $month);

		$fixed_total = 0;
		if ($staff_pay) {
			$food = floatval($staff_pay->food_allowance ?? 0);
			$allowance = floatval($staff_pay->allowance ?? 0);
			$fat = floatval($staff_pay->fat_allowance ?? 0);
			$mewa = floatval($staff_pay->mewa ?? 0);

			$fixed_total = ($food + $allowance + $fat + $mewa) * $proration_factor;

			// Debug logging
			log_activity("GOSI Other Calc - Staff: $staff_id, Month: $month, Pay ID: {$staff_pay->id}, Food: $food, Allow: $allowance, FAT: $fat, MEWA: $mewa, Fixed Total: $fixed_total (Proration: " . round($proration_factor, 4) . ")");
		} else {
			log_activity("GOSI Other Calc - Staff: $staff_id, Month: $month - NO PAY RECORD FOUND");
		}

		// Get custom allowances
		$custom_allowances = $this->get_employee_applicable_allowances($staff_id);
		$custom_total = 0;

		if ($staff_pay && !empty($custom_allowances)) {
			foreach ($custom_allowances as $allowance) {
				// Get the amount from tblstaff_pay_allowances if exists
				...
				if ($custom_amount) {
					$amt = floatval($custom_amount->amount) * $proration_factor;
					$custom_total += $amt;
					log_activity("GOSI Other Calc - Custom Allowance {$allowance['name']} (ID: {$allowance['id']}): $amt (prorated)");
				} else if (!empty($allowance['default_amount'])) {
					// Use default amount if no custom amount set
					$amt = floatval($allowance['default_amount']) * $proration_factor;
					$custom_total += $amt;
					log_activity("GOSI Other Calc - Custom Allowance {$allowance['name']} (ID: {$allowance['id']}): $amt (default, prorated)");
				} else {
					log_activity("GOSI Other Calc - Custom Allowance {$allowance['name']} (ID: {$allowance['id']}): 0 (not set)");
				}
			}
		}

		$total = $fixed_total + $custom_total;
		log_activity("GOSI Other Calc - Staff: $staff_id, Total: $total (Fixed: $fixed_total + Custom: $custom_total)");
	 ...
	 * Includes: ALL fixed allowances (food, general, fat, accommodation, MEWA) + custom allowances
	 * @param int $staff_id Staff ID
	 * @param string $month Month in Y-m-d format (e.g., 2025-01-01)
	 * @return float Total allowances
	 */
	public function calculate_total_allowance($staff_id, $month, $proration_factor = 1.0)
	{
		// Get fixed allowances from tblstaffpay
		$staff_pay = $this->get_staff_pay_for_month($staff_id, $month);
		...
		$fixed_total = 0;
		if ($staff_pay) {
			$fixed_total = (floatval($staff_pay->food_allowance ?? 0) +
						  floatval($staff_pay->allowance ?? 0) +
						  floatval($staff_pay->fat_allowance ?? 0) +
						  floatval($staff_pay->accomodation_allowance ?? 0) +
						  floatval($staff_pay->mewa ?? 0)) * $proration_factor;  // ✅ MEWA included, prorated
		}

		// Get custom allowances (same logic as gosi_other)
		$custom_allowances = $this->get_employee_applicable_allowances($staff_id);
		$custom_total = 0;
		...
		if ($staff_pay && !empty($custom_allowances)) {
			foreach ($custom_allowances as $allowance) {
				...
				if ($custom_amount) {
					$custom_total += floatval($custom_amount->amount) * $proration_factor;
				} else if (!empty($allowance['default_amount'])) {
					$custom_total += floatval($allowance['default_amount']) * $proration_factor;
				}
			}
		}

		return $fixed_total + $custom_total;
	}

	/**
	 * Get payroll export data for selected staff members
	 * @param array $staff_ids Array of staff IDs to export
	 * @param string $month Month in Y-m format (e.g., 2025-01)
	 * @param string|null $company Company filter (mohtarifeen or mahiroon)
	 * @return array Array of employee data for export
	 */
	public function get_payroll_export_data($staff_ids, $month, $company = null)
	{
		...
		$month_date = date('Y-m-d', strtotime($month . '-01'));

		// Normalize month for array key lookup (Y-m format)
		$month_key = date('Y-m', strtotime($month_date));

		$export_data = [];

		foreach ($staff_ids as $staff_id) {
			// Get staff information (including ownemployee_id to verify permanent status)
			$this->db->select('s.staffid, s.name, s.firstname, s.lastname, s.iqama_number, s.bank_iban_number, s.bank_swift_code, s.basics, s.companytype_id, s.ownemployee_id');
			...
			// Double-check: Only export permanent employees
			// Load helper if not loaded
			if (!function_exists('get_ownemployee_type_id_by_name')) {
				$CI = &get_instance();
				$CI->load->helper('ownemployee_type');
			}
			$permanent_type_id = get_ownemployee_type_id_by_name('Permanent');
			if ($permanent_type_id && isset($staff_info->ownemployee_id) && (int)$staff_info->ownemployee_id !== $permanent_type_id) {
				// Skip non-permanent employees
				continue;
			}

			// Get employee payroll data for the month
			$this->db->select('*');
			$this->db->from(db_prefix() . 'hrp_employees_value');
			$this->db->where('staff_id', $staff_id);

			// Try multiple month formats
			$this->db->group_start();
			$this->db->or_where('month', $month_date); // Format: 2025-01-01
			$this->db->or_where('month', date('m', strtotime($month_date))); // Format: 01
			$this->db->or_where('month', $month_key); // Format: 2025-01
			$this->db->group_end();

			$payroll_data = $this->db->get()->row();
			...
			if ($payroll_data) {
				// Get values from saved payroll data
				$basic = floatval(str_replace(',', '', $payroll_data->basic ?? 0));
				$allowance = floatval(str_replace(',', '', $payroll_data->allowance ?? 0));
				$deduction = floatval(str_replace(',', '', $payroll_data->deduction ?? 0));
				$ot_amount = floatval(str_replace(',', '', $payroll_data->ot_amount ?? 0));
				$full_salary = floatval(str_replace(',', '', $payroll_data->full_salary ?? 0));
				$balance = floatval(str_replace(',', '', $payroll_data->balance ?? 0));
			} else {
				// No saved payroll data - get from tblstaffpay
				$staff_pay = $this->get_staff_pay_for_month($staff_id, $month_date);

				// Get project additions/deductions
				$project_additions = $this->get_project_additions_for_month($staff_id, $month_date);
				$project_deductions = $this->get_project_deductions_for_month($staff_id, $month_date);

				if ($staff_pay) {
					$basic = floatval($staff_pay->basic_pay ?? 0);
					$food_allowance = floatval($staff_pay->food_allowance ?? 0);
					$allowance_pay = floatval($staff_pay->allowance ?? 0);
					$fat_allowance = floatval($staff_pay->fat_allowance ?? 0);
					$accomodation_allowance = floatval($staff_pay->accomodation_allowance ?? 0);
					$mewa = floatval($staff_pay->mewa ?? 0);

					$allowance = $allowance_pay + $food_allowance + $accomodation_allowance + $fat_allowance + $project_additions;
					$deduction = $mewa + $project_deductions;
				} else {
					// Ultimate fallback
					$basic = 0;
				...
				$ot_amount = 0; // No OT hours saved yet
			}

			// Calculate net amount
			// Net = Full Salary (if available) OR (Basic + OT + Allowances - Deductions)
			if ($full_salary > 0) {
				$net_amount = $full_salary;
			} else {
				$net_amount = $basic + $ot_amount + $allowance - $deduction;
			}

			// If balance exists, use it as net amount (more accurate)
			if ($balance > 0) {
				$net_amount = $balance;
			}

			// Get housing allowance and other earnings breakdown for WPS
			$housing_allowance = 0;
			$other_earnings = 0;

			if ($payroll_data) {
				// Get housing allowance from GOSI housing field
				$housing_allowance = floatval(str_replace(',', '', $payroll_data->gosi_housing_allowance ?? 0));

				// Other earnings = allowance field (which includes all other allowances except housing)
				$other_earnings = $allowance;
			} else if ($staff_pay) {
				// Calculate from staff pay
				$housing_allowance = floatval($staff_pay->accomodation_allowance ?? 0);

				// Other earnings = all other allowances
				$food = floatval($staff_pay->food_allowance ?? 0);
				$general = floatval($staff_pay->allowance ?? 0);
				$fat = floatval($staff_pay->fat_allowance ?? 0);
				$mewa = floatval($staff_pay->mewa ?? 0);
				$other_earnings = $food + $general + $fat + $mewa;
			}

			// Build export row
			$export_row = [
				'staff_id' => $staff_id,
				'employee_name' => $staff_info->name,
				'employee_id_iqama' => $payroll_data->employee_id_iqama ?? $staff_info->iqama_number ?? '',
				'employee_account_no_iban' => $payroll_data->employee_account_no_iban ?? $staff_info->bank_iban_number ?? '',
				'bank_code' => $payroll_data->bank_code ?? $staff_info->bank_swift_code ?? '',
				...
				'zipcode' => '', // Not stored in current schema
			];
	...
	public function get_payroll($payroll_id)
	{
		...
		return $this->db->get(db_prefix() . 'hrp_payroll')->row();
	}

	/**
	 * Get payroll with full details (company name, employee type name, creator)
	 * @param int $payroll_id
	 * @return object|null
	 */
	public function get_payroll_details($payroll_id)
	{
		$this->db->select('p.*, CONCAT(s.firstname, " ", s.lastname) as created_by_name, ct.name as company_name, oet.name as ownemployee_type_name');
		$this->db->from(db_prefix() . 'hrp_payroll p');
		$this->db->join(db_prefix() . 'staff s', 's.staffid = p.created_by', 'left');
		$this->db->join(db_prefix() . 'companytype ct', 'ct.id = p.company_filter', 'left');
		$this->db->join(db_prefix() . 'ownemployeetype oet', 'oet.id = p.ownemployee_type_id', 'left');
		$this->db->where('p.id', $payroll_id);
		return $this->db->get()->row();
	}
	 ...
	 * @return array
	 */
	public function get_payroll_employees($payroll_id)
	{
	...
	/**
	 * Get single employee payroll record
	 * @param int $payroll_id
	 * @param int $staff_id
	 * @return object|null
	 */
	public function get_employee_payroll_record($payroll_id, $staff_id)
	{
		$this->db->where('payroll_id', $payroll_id);
		$this->db->where('staff_id', $staff_id);
		return $this->db->get(db_prefix() . 'hrp_employees_value')->row();
	}
	 ...
	 * @return bool
	 */
	public function update_employee_payroll_record($payroll_id, $staff_id, $data)
	{
		$this->db->where('payroll_id', $payroll_id);
		$this->db->where('staff_id', $staff_id);
		$this->db->update(db_prefix() . 'hrp_employees_value', $data);

		// Check for database errors
		$db_error = $this->db->error();
		if ($db_error['code'] != 0) {
			log_activity("Update Employee Payroll Record Error - Payroll ID: {$payroll_id}, Staff ID: {$staff_id}, Error: " . $db_error['message']);
			return false;
		}

		// Return true if update succeeded (affected_rows can be 0 if data is unchanged, which is still success)
		return true;
	}

	/**
	 * Get payrolls with optional filtering
	 * @param array $filters - month, status, company_filter, ownemployee_type_id
	 * @return array
	 */
	public function get_payrolls($filters = [])
	{
		$this->db->select('p.*, CONCAT(s.firstname, " ", s.lastname) as created_by_name, ct.name as company_name, oet.name as ownemployee_type_name');
		$this->db->from(db_prefix() . 'hrp_payroll p');
		$this->db->join(db_prefix() . 'staff s', 's.staffid = p.created_by', 'left');
		...
		if (!empty($filters['company_filter'])) {
			$this->db->where('p.company_filter', $filters['company_filter']);
		}

		if (!empty($filters['ownemployee_type_id'])) {
			$this->db->where('p.ownemployee_type_id', $filters['ownemployee_type_id']);
		}

		if (!empty($filters['month_from'])) {
			$this->db->where('p.month >=', $filters['month_from']);
		}

		if (!empty($filters['month_to'])) {
			$this->db->where('p.month <=', $filters['month_to']);
		}
	 ...
	 * @param string $month - YYYY-MM-DD format
	 * @param int $company_filter - Company ID (1=Mahiroon, 2=Mohtarifeen) or null
	 * @param int $ownemployee_type_id
	 * @return string
	 */
	public function generate_payroll_number($month, $company_filter, $ownemployee_type_id)
	{
		// Format: PR-{YEAR}{MONTH}-{COMPANY}-{TYPE}-{SEQUENCE}
		// Example: PR-202510-MOH-PERM-001

		$year_month = date('Ym', strtotime($month));

		// Company code (based on company ID)
		$company_code = 'ALL';
		if ($company_filter == 1) {
			$company_code = 'MAH'; // Mahiroon
		} elseif ($company_filter == 2) {
			$company_code = 'MOH'; // Mohtarifeen
		}

		// Employee type code (first 4 letters of type name)
		$type_name = get_ownemployee_type_name($ownemployee_type_id);
		$type_code = strtoupper(substr($type_name, 0, 4));

		// Get sequence number for this month
		$this->db->select('COUNT(*) as count');
		$this->db->from(db_prefix() . 'hrp_payroll');
		$this->db->where('YEAR(month)', date('Y', strtotime($month)));
		$this->db->where('MONTH(month)', date('m', strtotime($month)));
		$result = $this->db->get()->row();
		$sequence = str_pad(($result->count + 1), 3, '0', STR_PAD_LEFT);

		return "PR-{$year_month}-{$company_code}-{$type_code}-{$sequence}";
	}
	...
	public function create_payroll($data)
	{
		...
		if ($existing) {
			return false; // Payroll already exists
		}

		// Generate payroll number
		$data['payroll_number'] = $this->generate_payroll_number(
			$data['month'],
			$data['company_filter'],
			$data['ownemployee_type_id']
		);

		// Get employee type name
		$data['ownemployee_type_name'] = get_ownemployee_type_name($data['ownemployee_type_id']);
		...
		// Insert payroll
		$this->db->insert(db_prefix() . 'hrp_payroll', $data);
	...
	/**
	 * Generate payroll employees from staff_pay data
	 * @param int $payroll_id
	 * @param string $month - YYYY-MM-DD format
	 * @param string $company_filter
	 * @param int $ownemployee_type_id
	 * @param string $rel_type
	 * @return int - Number of employees added
	 */
	public function generate_payroll_employees($payroll_id, $month, $company_filter, $ownemployee_type_id, $rel_type)
	{
		// Calculate month boundaries
		$month_start = date('Y-m-01', strtotime($month));
		$month_end = date('Y-m-t', strtotime($month));

		// Build query to get eligible employees
		// Include ALL active employees, we'll handle start dates in the processing loop
		$this->db->select('s.*, sp.*, CONCAT(s.firstname, " ", s.lastname) as full_name');
		$this->db->from(db_prefix() . 'staff s');
		$this->db->join(db_prefix() . 'staffpay sp', 'sp.staff_id = s.staffid', 'left');
		$this->db->where('s.active', 1);
		$this->db->where('s.ownemployee_id', $ownemployee_type_id);
		$this->db->where_not_in('s.staffid', [1, 3]); // Exclude system accounts

		// Company filter (company_filter is now company ID: 1=Mahiroon, 2=Mohtarifeen)
		if (!empty($company_filter)) {
			$this->db->where('s.companytype_id', $company_filter);
		}

		$employees = $this->db->get()->result_array();

		// DEBUG: Log query and count
		log_activity('Payroll Generation Query: ' . $this->db->last_query());
		log_activity('Payroll Generation - Found ' . count($employees) . ' employees for payroll_id=' . $payroll_id);

		$added_count = 0;

		foreach ($employees as $employee) {
			// Get staff_id for adjustments lookup
			...
			if ($start_date) {
				...
				if (strtotime($start_date) > strtotime($month_end)) {
					...
					$proration_factor = 0;
					log_activity("Payroll Generation - Staff {$staff_id}: Future start date {$start_date} (after {$month_end}), showing zero amounts");
				}
				...
				elseif (strtotime($start_date) >= strtotime($month_start) && strtotime($start_date) <= strtotime($month_end)) {
					$start_day = (int)date('j', strtotime($start_date)); // Day of month (1-31)
					$working_days = $total_days_in_month - $start_day + 1; // Include start day
					$proration_factor = $working_days / $total_days_in_month;

					// Log proration for debugging
					log_activity("Payroll Proration - Staff {$staff_id}: Started {$start_date}, Working {$working_days}/{$total_days_in_month} days, Factor: " . round($proration_factor, 4));
				}
				...
				log_activity("Payroll Generation - Staff {$staff_id}: No pay record found, showing zero amounts");
			}

			// Get additions/deductions from tblhrp_adjustments table
			$hrp_additions = $this->get_additions_for_month($staff_id, $month);
			$hrp_deductions = $this->get_deductions_for_month($staff_id, $month);

			// Get basic pay with proration
			$basic_pay = floatval($employee['basic_pay'] ?? 0) * $proration_factor;

			// Calculate GOSI totals from staff_pay with corrected mappings and apply proration
			$gosi_basic = floatval($employee['gosi_basic'] ?? $employee['basic_pay'] ?? 0) * $proration_factor;
			$gosi_housing = floatval($employee['gosi_housing_allowance'] ?? $employee['accomodation_allowance'] ?? 0) * $proration_factor;
			$gosi_other = $this->calculate_gosi_other_allowance($staff_id, $month, $proration_factor);
			$gosi_deduction = 0;  // MEWA is now an allowance, not a deduction

			// Get timesheet OT for this month (if any)
			// TODO: Implement timesheet OT calculation
			$ot_hours = 0;
			$ot_rate = $employee['overtime_pay'] ?? 0;
			$ot_amount = $ot_hours * $ot_rate;

			// Calculate total allowance (for payslip display)
			$total_allowance = $this->calculate_total_allowance($staff_id, $month, $proration_factor);
			...
			$employee_data = [
				...
				'ot_rate' => $ot_rate,
				'ot_amount' => $ot_amount,
				'allowance' => $this->calculate_total_allowance($staff_id, $month, $proration_factor),  // All allowances including custom, prorated
				'additions' => $hrp_additions,  // HRP additions from tblhrp_adjustments
				'deduction' => $hrp_deductions,  // Only HRP deductions from tblhrp_adjustments
				'full_salary' => $full_salary,
				'balance' => $balance,
				'mention' => '',
				'probationary_contracts' => json_encode([]),
				'primary_contracts' => json_encode([]),
			];

			// Insert employee record
			$this->db->insert(db_prefix() . 'hrp_employees_value', $employee_data);
			...
			if ($this->db->affected_rows() > 0) {
				...
				$db_error = $this->db->error();
				log_activity('Payroll Generation - Failed to insert employee ' . $employee['staffid'] . ': ' . $db_error['message']);
			}
		}

		// DEBUG: Log final count
		log_activity('Payroll Generation - Successfully added ' . $added_count . ' employees to payroll_id=' . $payroll_id);

		// Update payroll totals
		$this->update_payroll_totals($payroll_id);

		return $added_count;
	}

	/**
	 * Update payroll totals (employee count and total amount)
	 * @param int $payroll_id
	 * @return bool
	 */
	public function update_payroll_totals($payroll_id)
	{
		$this->db->select('COUNT(*) as total_employees, SUM(full_salary) as total_amount');
		$this->db->from(db_prefix() . 'hrp_employees_value');
		$this->db->where('payroll_id', $payroll_id);
		$totals = $this->db->get()->row();

		$this->db->where('id', $payroll_id);
		$this->db->update(db_prefix() . 'hrp_payroll', [
	 ...
	 * @param array $data - payroll_id, status, updated_by, updated_date fields
	 * @return bool
	 */
	public function update_payroll_status($data)
	{
		$payroll_id = $data['payroll_id'];
		unset($data['payroll_id']);

		$this->db->where('id', $payroll_id);
		$this->db->update(db_prefix() . 'hrp_payroll', $data);
	...
	public function delete_payroll($payroll_id)
	{
		...
		$this->db->where('payroll_id', $payroll_id);
		$this->db->delete(db_prefix() . 'hrp_project_payments');

		// Delete status log
		$this->db->where('payroll_id', $payroll_id);
		$this->db->delete(db_prefix() . 'hrp_payroll_status_log');

		// Delete payroll
		$this->db->where('id', $payroll_id);
		$this->db->delete(db_prefix() . 'hrp_payroll');
	...
	/**
	 * Log status change
	 * @param array $data - payroll_id, from_status, to_status, changed_by, comments
	 * @return bool
	 */
	public function log_status_change($data)
	{
		...
		$this->db->insert(db_prefix() . 'hrp_payroll_status_log', $data);

		return $this->db->affected_rows() > 0;
	}

	/**
	 * Get status change log for a payroll
	 * @param int $payroll_id
	 * @return array
	 */
	public function get_payroll_status_log($payroll_id)
	{
		$this->db->select('l.*, CONCAT(s.firstname, " ", s.lastname) as changed_by_name');
		$this->db->from(db_prefix() . 'hrp_payroll_status_log l');
		$this->db->join(db_prefix() . 'staff s', 's.staffid = l.changed_by', 'left');
		$this->db->where('l.payroll_id', $payroll_id);
		$this->db->order_by('l.changed_date', 'ASC');
