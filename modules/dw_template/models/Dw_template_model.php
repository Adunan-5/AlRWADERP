...
class Dw_template_model extends App_Model
{
    protected $documents_table = 'tbl_dw_documents';
    protected $types_table = 'tbl_dw_types';
    protected $letterheads_table = 'tbl_letterheads'; // Reuse existing
    ...
    {
        return $this->db->select('tbl_dw_documents.*, tbl_dw_types.label as type_label')
                        ->join('tbl_dw_types', 'tbl_dw_types.id = tbl_dw_documents.type_id', 'left')
                        ->order_by('tbl_dw_documents.created_at', 'DESC')
                        ->get($this->documents_table)
                        ->result_array();
    }
    ...
    public function generate_pdf($document_id)
    {
        ...
        $folder = 'uploads/dw_documents/' . date('Y') . '/' . date('m') . '/';
